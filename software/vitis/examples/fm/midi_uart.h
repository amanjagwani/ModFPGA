			...
			if (expected > 0 && data_count >= expected)
			{
				...
				if (status == 0x90)
				{
					...
					if (velocity > 0)
					{
						...
						note_on(note, amp, synth_state.mod_ratio);
					}
					else
					{
						note_off(note);
				...
				else if (status == 0xB0)
				{
					...
					if (cc_number == 74)
					{
						synth_state.mod_index = map_range(cc_value, 0, 127, 0, 10);
						write_indx(synth_state.mod_index);
					}
					else if (cc_number == 71)
					{
						synth_state.mod_ratio = map_range(cc_value, 0, 127, 1, 5);
					}
					else if (cc_number == 76)
					{
						synth_state.mod_adsr[0] = map_range(cc_value, 0, 127, 0.0000001, 10.0000000);
						set_amp_env(synth_state.mod_adsr[0], synth_state.mod_adsr[1], synth_state.mod_adsr[2], synth_state.mod_adsr[3]);
					}
					...
					{
						synth_state.mod_adsr[1] = map_range(cc_value, 0, 127, 0.0000001, 10.0000000);
						set_amp_env(synth_state.mod_adsr[0], synth_state.mod_adsr[1], synth_state.mod_adsr[2], synth_state.mod_adsr[3]);
					}
					else if (cc_number == 78)
					{
						synth_state.mod_adsr[2] = map_range(cc_value, 0, 127, 0.00000001, 1.0000);
						set_amp_env(synth_state.mod_adsr[0], synth_state.mod_adsr[1], synth_state.mod_adsr[2], synth_state.mod_adsr[3]);
					}
	...
	if (XUartPs_IsReceiveData(uart_instance->Config.BaseAddress))
	{
		...
		if (data == '\n' || data == '\r')
		{
			...
			if (sscanf(buffer, "Note On: %d, Velocity: %d", &note, &velocity) == 2)
			{
				if (velocity > 0)
				{
					note_on(note, map_range(velocity, 0, 127, 0.000, 0.900), synth_state.mod_ratio);
				}
			}
