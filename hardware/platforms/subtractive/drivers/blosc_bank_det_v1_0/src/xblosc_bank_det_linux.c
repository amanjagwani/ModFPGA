...
typedef struct {
    ...
    XBlosc_bank_det_uio_map maps[ MAX_UIO_MAPS ];
} XBlosc_bank_det_uio_info;

/***************** Variable Definitions **************************************/
static XBlosc_bank_det_uio_info uio_info;
...
static int uio_info_read_name(XBlosc_bank_det_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBlosc_bank_det_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBlosc_bank_det_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBlosc_bank_det_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBlosc_bank_det_Initialize(XBlosc_bank_det *InstancePtr, const char* InstanceName) {
	XBlosc_bank_det_uio_info *InfoPtr = &uio_info;
